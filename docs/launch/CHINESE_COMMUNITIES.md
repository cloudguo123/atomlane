...
AtomLane 的做法更像一个小编译器：先把支持的 shell、package scripts、Make、Compose、测试和构建入口降到带类型的 Atom IR，保留成功、失败、顺序、数据、流、就绪、健康、完成和清理关系，再根据文件副作用、逻辑资源、生命周期和 Mac 当前资源决定谁能同时运行。未知副作用直接拒绝，不靠猜。
