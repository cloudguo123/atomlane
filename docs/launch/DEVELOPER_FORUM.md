...
AtomLane: parallelize only what is proven safe in Codex on macOS
...
AtomLane treats planning like compilation. It parses supported shell/package/Make/Compose/test entrypoints into a typed Atom IR, keeps success/failure/data/lifecycle edges distinct, models artifacts and resource capacity, and executes only the exact hashed plan. Unknown effects fail closed. For runs over ten seconds, a PTY view shows running/ready/completed/failed counts and current estimated savings instead of a blank spinner.
