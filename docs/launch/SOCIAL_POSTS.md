...
I built AtomLane: one universal safety core, platform-native execution, and workload-tailored acceleration—parallelizing local work only when independence is proven.

macOS Stable · native Windows Preview · typed atomic plans · fail-closed effects · platform-aware resources · Windows Job containment · live progress · per-run + cumulative savings.
...
I made a local Codex plugin for the part of parallel execution that usually worries me: preserving semantics. One shared typed core keeps success/failure/data/lifecycle edges, artifacts, hashes, and capacity consistent; adapters tailor execution to the workload and real platform. macOS is Stable for the supported shell/package/Make/Compose/test frontends; native Windows has a scoped Preview for exact argv and declared PowerShell files, with Job Object containment and separate WSL/Docker realms. Unknown effects fail closed. Long runs stream real-time progress and savings.
