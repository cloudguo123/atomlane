...
const g=d.growth;if(!g.available){{q('#growth').innerHTML='<div class="growth-card"><strong>Pending</strong><span>First aggregate snapshot</span></div>';q('#growth-note').textContent='The weekly metrics workflow will publish stars, forks, release downloads, and GitHub’s rolling 14-day traffic counters.';}}else{{const x=g.latest,t=x.traffic_14d||{{}};const gm=[['Stars',x.stars],['Forks',x.forks],['14d unique visitors',t.unique_visitors??'—'],['14d unique cloners',t.unique_cloners??'—'],['Release downloads',x.release_asset_downloads],['Open issues + PRs',x.open_issues_and_prs],['30d star target',g.targets_30d.stars],['30d visitor target',g.targets_30d.unique_visitors]];q('#growth').innerHTML=gm.map(v=>`<div class="growth-card"><strong>${{v[1]}}</strong><span>${{v[0]}}</span></div>`).join('');const stale=x.traffic_stale_from?' Traffic last authenticated '+x.traffic_stale_from+'.':'';q('#growth-note').textContent='Captured '+x.captured_at+'.'+stale+' Traffic is a rolling 14-day GitHub window and may lag. Clones and downloads indicate intent, not verified installations.';}}
</script>
</body>
</html>
"""
